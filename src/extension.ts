...
const getCurrentUser = () =>
  vscode.workspace.getConfiguration()
    .get('rubberduckheader.username') || process.env['USER'] || 'duckie'
...
const getCurrentUserMail = () =>
  vscode.workspace.getConfiguration()
    .get('rubberduckheader.email') || `${getCurrentUser()}`
...
export const activate = (context: vscode.ExtensionContext) => {
  const disposable = vscode.commands
    .registerTextEditorCommand('rubberduckheader.insertHeader', insertHeaderHandler)
